...
sub-scores from clinical data. It works on both cross-sectional
snapshots and longitudinal (one-row-per-day) layouts, and supports SI as
well as conventional units. Thresholds follow Vincent et al. (1996).

## Highlights

- Six vectorised component scorers
...
|-------------------------------------------------------------------------------------------------|-------------------------------------------------|
| [`score_respiratory()`](https://ksa98.github.io/sofamatic/reference/score_respiratory.md)       | PaO2/FiO2 (kPa or mmHg), with ventilation flag  |
| [`score_coagulation()`](https://ksa98.github.io/sofamatic/reference/score_coagulation.md)       | Platelet count                                  |
| [`score_liver()`](https://ksa98.github.io/sofamatic/reference/score_liver.md)                   | Bilirubin (µmol/L or mg/dL)                     |
| [`score_cardiovascular()`](https://ksa98.github.io/sofamatic/reference/score_cardiovascular.md) | MAP plus dopamine / dobutamine / NE / E doses   |
| [`score_neurological()`](https://ksa98.github.io/sofamatic/reference/score_neurological.md)     | GCS, with optional sedation handling            |
| [`score_renal()`](https://ksa98.github.io/sofamatic/reference/score_renal.md)                   | Creatinine (µmol/L or mg/dL), urine, dialysis   |
| [`calculate_sofa()`](https://ksa98.github.io/sofamatic/reference/calculate_sofa.md)             | All of the above + total + longitudinal helpers |

Every component returns an integer vector in `{0, 1, 2, 3, 4}` (or
`NA`).
[`calculate_sofa()`](https://ksa98.github.io/sofamatic/reference/calculate_sofa.md)
returns the input data frame augmented with one column per sub-score,
the total `SOFA_score`, and (when `id`/`time` are supplied)
...
Pass `NULL` for any optional column you don’t have. For example, if your
dataset has no urine output and no dialysis flag, the renal sub-score is
derived from creatinine alone:
...
treated as moderate impairment (score 2). Override with `na_strategy` if
...
expected by
[`calculate_sofa()`](https://ksa98.github.io/sofamatic/reference/calculate_sofa.md).
Values are drawn from a simple parametric generator:

- **Per-subject statics** are drawn once and broadcast over the
  subject’s days: `age ~ Normal(62, 12)`, sex sampled with `P(M) = 0.6`,
  `bmi ~ Normal(27, 5)`.
- **Per-row flags** are independent Bernoullis: ventilation `P = 0.85`,
  sedation `P = 0.55`, vasopressor use `P = 0.40`, dialysis `P = 0.08`.
- **Conditional measurements** depend on those flags. PaO2/FiO2 (kPa) is
  drawn from `Normal(25, 8)` for ventilated rows and `Normal(50, 8)`
  otherwise (floored at 5). MAP is drawn from `Normal(70, 10)` on
  vasopressors and `Normal(80, 10)` off them. Norepinephrine dose is
  `Normal(0.06, 0.05)` µg/kg/min on vasopressor rows (floored at 0.001)
...
illustrative rather than derived from any specific cohort. Use the data
for examples, tests, and benchmarks — not for clinical inference.
...
- **[Case study: SOFA in a synthetic ICU
  cohort](https://ksa98.github.io/sofamatic/articles/case-study-icu-cohort.html)**
  — a worked applied analysis from raw data to deterioration metrics.
- **[Edge cases and
  limits](https://ksa98.github.io/sofamatic/articles/edge-cases-and-limits.html)**
  — boundary arithmetic, unit-conversion gotchas, missing-data policies,
  and a defensive scoring wrapper.
- **[FAQ](https://ksa98.github.io/sofamatic/articles/faq.html)** — short
...
read
[`CONTRIBUTING.md`](https://ksa98.github.io/sofamatic/CONTRIBUTING.md)
for the workflow, and open issues at
<https://github.com/ksa98/sofamatic/issues>. Issue templates for bugs
and feature requests are provided.
...
[`CITATION.cff`](https://ksa98.github.io/sofamatic/CITATION.cff) file,
...
[LICENSE](https://ksa98.github.io/sofamatic/LICENSE).
